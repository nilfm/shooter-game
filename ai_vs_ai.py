...
class Controller:
    ...
    def decision(self):
        for bullet in sorted(self.enemy.bullets, key=lambda b: distance_sq(b.pos, self.warrior.pos)):
            ...
            if (distance_sq(bullet_copy.pos, self.warrior.pos) <= self.warrior.radius**2):
                new_dists = []
        ...
        if self.warrior.cooldown or random.randint(0, self.warrior.scope_angle):
            return CLOSE_AIM
        else:
            return SHOOT
...
class App:
    def __init__(self):
        ...
        self.ai = [Controller(self.warriors[0], self.warriors[1]), Controller(self.warriors[1], self.warriors[0])]
    ...
    def handle_ai(self):
        for ai, warrior in zip(self.ai, self.warriors):
            aim = ai.aim()
            warrior.update_aim_ai(aim)
